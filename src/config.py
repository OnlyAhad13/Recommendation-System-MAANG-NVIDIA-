...
class ModelConfig:
    ...
    learning_rate_retrieval: float = 0.001  # Higher for retrieval focus
    ...
    num_hard_negatives: int = 5  # Increased
    num_random_negatives: int = 10  # Increased
    negative_sampling_strategy: str = "mixed"  # Mix is better
    
    # Multi-task weights (retrieval implicitly has weight 1.0)
    retrieval_weight: float = 1.0  # Pure retrieval
    ctr_weight: float = 2.0
    ...
    def __post_init__(self):
        if self.user_tower_dims is None:
            self.user_tower_dims = [256, 128, 64]  # Simpler tower
        if self.item_tower_dims is None:
            self.item_tower_dims = [256, 128,64]  # Simpler tower
        if self.dnn_dims is None:
            self.dnn_dims = [256, 128]  # Much simpler DCN
